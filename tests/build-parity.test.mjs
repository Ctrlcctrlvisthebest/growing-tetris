...
test('GitHub and Toy ship identical gameplay, moderation and translations with different language defaults', async () => {
  ...
  const shared = ['index.html', 'style.css', 'renderer.js', 'sketch.js', 'score-history.js', 'leaderboard.js', 'toy-leaderboard.js', 'leaderboard-config.js', 'i18n.js'];
  try {
    ...
    assert.match(await readFile(path.join(temp, 'dist-toy/platform-config.js'), 'utf8'), /'toy'/);
    assert.match(await readFile(path.join(temp, 'dist/platform-config.js'), 'utf8'), /'cloudflare'/);
